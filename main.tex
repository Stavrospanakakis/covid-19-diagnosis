...
Except from reviewing papers, we decided to create a custom network based on ResNet-101\cite{resnet}. Here will be many information about ResNet. ResNet is trained using ImageNet 2012 Dataset.\cite{imagenet} For the creation of Resnet network, two models were trained, a plain and a residual. The plain network is inspired by the philosophy of VGG networks.\cite{vgg} The convolutional layers have mostly 3x3 filters, The network ends with global averages pooling layer and 1000-way-fully-connected layer with softmax. The total number of
weighted layers is 34. Residual Network is based on the plain network mentioned above. It adds shortcut connections which turn the network into its counterpart residual version. ////The identity
...
The network we used is a residual one with 101 layers. For the creation of ResNet-101 was used more 3-layer blocks. Is is more accurate from the plain ones. 


 About the dataset, imagenet 2012 has 1000 classes and the models is trained at 1.28 million images and evalutated on 50k validation images. Image were cropped to 224×224 randomly sampled from an image or its horizontal flip, with the per-pixel mean subtracted. It was also used standard color augmentation.\cite{imagenet} It was also again used batch normalization\cite{batchnormalization} right after each convolution and before activation, following. The optimizer was SGD with a mini-batch size of 256.The learning rate starts from 0.1 and is divided by 10 when the error plateaus, and the models are trained for up to 60 × 104 iterations. It was 
used a weight decay of 0.0001 and a momentum of 0.9. It was not used dropout.\cite{dropout}

\begin{figure}[H]
\includegraphics[width=\linewidth]{assets/resnet-vs-vgg.png}
  \caption{VGG vs ResNet}
  \label{fig:vggvsresnet}
\end{figure}

\subsection{Datasets}
The dataset used for healthy and pneumonia chest x-ray images is from Kaggle created by Paul Mooney.\cite{dataset1custom}. It contains 5,863 images. The datasets used for COVID-19 are 
The datasets which used were "A COVID-19 X-ray image database" which was developed by Cohen JP\cite{dataset1secondreview} and "COVID-19 Chest X-ray Dataset Initiative"\cite{datasetcustomcovid} as well as "Actualmed COVID-19 Chest X-ray Dataset Initiative"\cite{datasetcustomcovid}. The total images are 6432. The 80 percent of the data used for train and the 20 percent for validation. 

\subsection{Image proccesing}
The image proccesing of the images is to resize them to 128x128, Shuffle them and normalize them by dividing with 255. 

\subsection{Paramaters}
The input shape of the network is (5144, 128, 128 3), the dataset shuffled after every epoch, the batch size is 16, the optimizer is Adam and the learning rate is 0.01. 

We created two versions of the network. The one which was CovidvsNonCovid and the other was CovidvsHealthyvsPneumonia.

\subsection{COVID vs Healthy vs Pneumonia}

The Loss function which was used is Categorical Crossentropy and the Last’s layer function is Softmax because of the fact that they were 3 different labels.

The total parameterss of the network are 76,216,707, the trainable parameters are 33,558,531 and the non-trainable 42,658,176

The accuracy was 91,5 percent
...
The Loss function below is Binary Crossentropy and the Last’s layer function is Sigmoid because of the fact that they are 2 different labels.

The total parameterss of the network are 76,214,657, the trainable parameters are 33,556,481 and the non-trainable 42,658,176.

The accuracy was 97,4 percent
...
\begin{table}[H]
  \begin{center}
    ...
    \begin{tabular}{||c c c c c c ||}
      \hline 
      \textbf{Reference} & \textbf{Problem} & \textbf{Method} & \textbf{Types of data} & \textbf{Patients / Sample size} & \textbf{Accuracy}\\ 
      \hline
...
\section{Future Work}
The future work might be a informative system. It could contains a mobile app which scans Chest x-rays, will classify them and send them to a cloud server with their appropriate information ( Disease, Patient ID, Age, Gender). The new x-rays could help for the continuum training of the COVnet-101 to help to be more accurate. At the end of the day a huge open dataset will have been created in order to help more scientists to create their custom networks. 
